# import → przygotowanie danych → podstawowa analiza danych → wizualizacja → eksport
# 
# Pliki do zadania
# W folderze znajdują się pliki CSV zawierające dane makroekonomiczne krajów świata, pochodzące z Banku Światowego z 2016 r.:
#   • kraje_makro_1.csv - podstawowe dane (m.in. PKB, populacja, przyrost populacji).
# • kraje_makro_2.csv - dane uzupełniające (m.in. region świata, urbanizacja, dostęp do internetu).
# Zawartość obu plików jest różna, pomimo identycznej ilości wierszy i kolumn - jest to celowe.
# Zmienne plik 1:
#   Panstwo - nazwa kraju (po angielsku)
# Kod - trzyliterowy kod ISO kraju
# PKB - Produkt Krajowy Brutto w USD (wartości nominalne)
# Populacja - liczba mieszkańców
# Przyrost_populacji - roczny przyrost populacji w procentach
# 
# Zmienne plik 2:
#   Country_Code - trzyliterowy kod ISO kraju
# Country_Name - nazwa kraju (po angielsku)
# World_region - region świata, do którego należy kraj (po angielsku)
# Urban.population.%.of.total. - procent ludności miejskiej
# Internet.access.%.of.total. - procent ludności z dostępem do Internetu
# 
# Kolejne etapy analityczne, które wykonasz na plikach
# 1.	import danych z dwóch plików CSV jako ramek danych
# 2.	przygotowanie danych: podgląd danych, porządkowanie nazw, typów, braków, czyszczenie danych
# 3.	scalenie obu ramek danych w jedną (analogia WYSZUKAJ.PIONOWO w Excelu)
# 4.	podstawowa analiza z użyciem pakietu dplyr: mutate/filter/select/arrange/group_by/summarise
# 5.	wizualizacja [* zaawansowane *] z użyciem pakietu ggplot2
# 6.	eksport ramki danych po obliczeniach do pliku CSV i Excel oraz eksport wykresów
